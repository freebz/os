...
void sheet_updown(struct SHEET *sht, int height)
{
  ...
  if (old > height) {	/* 이전보다 낮아진다. */
	...
	for (h = old; h < ctl->top; h++){
      ...
      sheet_refreshmap(ctl, sht->vx0, sht->vy0, sht->vy0 + sht->bxsize, sht->vy0 + sht->bysize, 0);
      sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vy0 + sht->bxsize, sht->vy0 + sht->bysize, 0, old - 1);
    }
