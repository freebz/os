...
void cons_putchar(struct CONSOLE *cons, int chr, char move)
{
  ...
  if (s[0] == 0x09) { /* 탭 */
    ...
    if (move != 0) {
      /* move가 0일 때는 커서를 움직이지 않는다. */
      cons->cur_x += 8;
      if (cons->cur_x == 8 + 240) { /* 오른쪽 끝까지 왔으므로 개행 */
	cons_newline(cons);
      }
    }
  }
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
  ...
  if (finfo == 0 && name[i -1] != '.') {
    /* 발견되지 않았기 때문에 뒤에 ".HRB"를 붙여 한 번 더 찾아본다. */
    name[i    ] = '.';
    name[i + 1] = 'H';
    name[i + 2] = 'R';
    name[i + 3] = 'B';
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx,
	     int ecx, int eax)
{
  ...
  if (edx == 1) {
    ...
    sheet_updown(sht, 3);	/* 높이 3은 task_a의 위 */
    reg[7] = (int) sht;
  } else if (edx == 6) {
